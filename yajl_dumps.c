...
int dump_tag(yajl_gen gen, const char *name, const int tag_mask, Monitor *mon) {
  // clang-format off
  YMAP(
...
int dump_tags(yajl_gen gen, const char *tags[], int tags_len, Monitor *mon) {
  // clang-format off
  YARR(
...
int dump_client(yajl_gen gen, Client *c) {
  ...
  YMAP(
      ...
      YSTR("pid"); YINT(c->pid);
      if (c->swallowing->name) {
        YSTR("swallowing"); YSTR(c->swallowing->name);
      }
      ...
      YSTR("geometry"); YMAP(
        ...
        YSTR("size_hints"); YMAP(
            ...
            YSTR("states"); YMAP(
			          ...
			          // YSTR("is_bar"); YBOOL( strcmp( c->name, altbartitle ) == 0 ? 1 : 0 );
			          )
            )
  // clang-format on
...
int dump_monitor(yajl_gen gen, Monitor *mon, int is_selected) {
  ...
  YMAP(
      ...
      YSTR("clients"); YMAP(
        ...
        YSTR("bar"); YMAP(
            YSTR("bar class"); YSTR(altbarclass);
